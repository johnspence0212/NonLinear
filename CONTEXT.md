...
The UI can hand work to the Cursor CLI (`agent`). Settings lists models from `agent models` and stores the chosen one. The repo is the git root of the directory NonLinear was started in. **to spec** on a map runs `/to-spec`. **to plan** on a map runs `/to-tickets`. **approve spec** approves, then runs `/to-tickets`. An open unblocked ticket can be **sent to cursor**.
