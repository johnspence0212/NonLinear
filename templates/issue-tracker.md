...
- **Project status**: `get_project_status` with `id` or `query` (`P-6`, numeric id, or title). Returns compact JSON (`kind: nonlinear.project-status`): derived stage, destination, progress counts, map/spec/plan summaries without bodies, frontier / claimed / blocked tickets, `next` (same rule as `list_frontier`), and `nextAction` (suggested MCP tool + id). Use this when asked what is going on with a project. Use `get_project` / `get_issue` to zoom into bodies.
